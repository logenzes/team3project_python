...
myPath = 'C:\dev\pythonProject'
# load in our YOLOv4 architecture network
network, class_names, class_colors = load_network("{}/ObjectDetection/darknet/cfg/yolov4-csp.cfg".format(myPath), "{}/ObjectDetection/darknet/cfg/coco.data".format(myPath), "{}/ObjectDetection/darknet/yolov4-csp.weights".format(myPath))
...
# 아래는 flutter랑 MQTT 서버에서 작동하기 떄문에 생략함===============================
...
def new_record(d, l):
  ...
  while d['r_finished'] == 0:
    if d['recording'] == 1:  # 레코딩중일 때는 계속 사진 찍기
      ...
      cv2.imwrite('{}/ObjectDetection/darknet/content/{}/{}.jpg'.format(myPath, d['detected_time'], title), d['img_color'])
...
def bounding_box(d, l):
  ...
  while d['r_finished'] == 0:
    ...
    if num_of_people > 0 and detected_person == 0:
      ...
      os.mkdir('{}/ObjectDetection/darknet/content/{}'.format(myPath, d['detected_time']))
      d['recording'] = 1
      detected_person = 1
...
def generate_video(d, l):
  ...
  def send_file(filename):
    files = open('{}/ObjectDetection/darknet/content/video/{}'.format(myPath, filename), 'rb')
    upload = {'file': files}
  ...
  while True:
    if d['video_total'] < d['detected_total']:
      ...
      # d['making'] = 1
      image_folder = '{}/ObjectDetection/darknet/content/{}'.format(myPath, l[d['video_total']])
      print("확인...", l[int(d['video_total'])],
            print(os.path.isdir('{}/ObjectDetection/darknet/content'.format(myPath))))
      images = [img for img in os.listdir(image_folder)]
      images.sort()
      print(images)
      video_name = '{}/ObjectDetection/darknet/content/video/{}.mp4'.format(
      ...
      video.release()
      d['recording'] = 0
      if os.path.exists('{}/ObjectDetection/darknet/content/{}'.format(myPath, l[d['video_total']])):
        shutil.rmtree('{}/ObjectDetection/darknet/content/{}'.format(myPath, l[d['video_total']]))
